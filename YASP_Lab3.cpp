...
//10 B. Реализовать шаблон класса вектор Vector<X>, последовательно хранящий элементы
//произвольного типа.Класс должен содержать методы, перечисленные в варианте 1.
//1. Реализовать шаблон класса односвязный(однонаправленный) список
//LinkedList<X>, хранящий элементы произвольного типа.Класс должен содержать
//методы :
...
//− Удаление одного элемента списка по значению;
//− Изменение всех элементов с данным значением на новое;
//− Удаление из списка совпадающих подряд идущих значений(оставить только
//	одно из всех одинаковых).
import Vector;
import Angle;
